...
const u8* portnum="9960";		 

//WIFI STA模式,设置要去连接的路由器无线参数,请根据你自己的路由器设置,自行修改.
const u8* wifista_ssid="0000";			//路由器SSID号
const u8* wifista_encryption="wpawpa2_aes";	//wpa/wpa2 aes加密方式
const u8* wifista_password="19990521"; 	//连接密码
...
u8 atk_8266_netpro_sel(void) 
{
	...
	while(1)
	{
		...
		if(key)
		{
			...
			if((key==KEY0_PRES)||(key==WKUP_PRES)) netpro=0;
...
void atk_8266_test(void)
{
	...
	printf("-----------%s-----------\r\n",(u8*)ATK_ESP8266_CWMODE_TBL[0]);  //STA模式 
